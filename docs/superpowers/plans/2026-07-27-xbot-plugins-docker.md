...
- `demo/.gitignore`: standard file generated by `xbot init`.
- `demo/README.md`: demo execution instructions matching the SSH demo layout.
- `demo/requirements.txt`: standard dependency file generated by `xbot init`.
...
- Modify: `README.md`
- Modify: `README.zh.md`
- Create through `xbot init`: `demo/.gitignore`
- Create: `demo/README.md`
- Create through `xbot init`: `demo/requirements.txt`
...
- [ ] **Step 3: Initialize the demo with xbot**

Run the xbot.framework 0.5.1 CLI:

```bash
/Users/zhaowcheng/Code/xbot/xbot.plugins.pgsql/venv/bin/xbot \
  init -d demo
```

Expected: `Initialized demo` and a standard xbot project containing
`.gitignore`, `README.md`, `requirements.txt`, `lib/`, `testbeds/`,
`testcases/`, and `testsets/`.

Use `apply_patch` to remove only the generated generic example files:

```text
demo/testbeds/testbed_example.yml
demo/testsets/testset_example.yml
demo/testcases/examples/__init__.py
```

If initialization copied `__pycache__` or `.pyc` files from the framework
template, do not add them to git.

- [ ] **Step 4: Create the demo connection cache**
...
- [ ] **Step 5: Create the demo testcase**
...
- [ ] **Step 6: Validate docs against the public API and commit**
